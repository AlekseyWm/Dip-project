...
export default function Login({ onLogin }) {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (!me.ok) throw new Error(`Не удалось получить current-user (${me.status})`);
      ...
      <div className="login-container">
        ...
        {isRegister ? (
          <>
            ...
            <button className="login-switch" onClick={() => setIsRegister(false)}>Уже есть аккаунт? Войти</button>
          </>
        ) : (
          <>
            ...
            <button className="login-switch" onClick={() => setIsRegister(true)}>Нет аккаунта? Зарегистрируйтесь</button>
          </>
        )}
      </div>
    </div>
